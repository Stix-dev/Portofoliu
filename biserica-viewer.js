import { constrainCameraMotion } from './biserica-camera-boundary.js';
...
export function mountChurchViewer(container, src) {
    ...
    function frame() {
        ...
        camera.updateProjectionMatrix();
        controls.update();
        safeCameraPosition.copy(camera.position);
        updateClipping();
    }
    function resize() {
        if (disposed) return;
    ...
    function render(time) {
        ...
        if (loaded && navigationBounds) {
            const next = constrainCameraMotion(safeCameraPosition.toArray(), camera.position.toArray(), navigationBounds, radius * .02);
            const correction = new THREE.Vector3().fromArray(next).sub(camera.position);
    ...
    function addShadowContrast(material) {
        material.onBeforeCompile = shader => {
            shader.uniforms.viewerShadowContrast = { value: .43 };
        ...
        material.customProgramCacheKey = () => 'church-shadow-contrast-v1';
    }
        ...
        try {
            ...
            model.traverse(object => {
                ...
                for (const material of materials) {
                    ...
                    if (material.isMeshStandardMaterial && !isGlass(material) && !material.transparent) addShadowContrast(material);
                }
            ...
            const azimuth = THREE.MathUtils.degToRad(111), altitude = THREE.MathUtils.degToRad(30);
