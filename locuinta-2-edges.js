...
function buildTopology(meshes) {
    ...
    for (const mesh of meshes) {
        ...
        for (let i = 0; i < count; i += 3) {
            ...
            if (normal.lengthSq() <= tolerance ** 4) { degenerate++; continue; }
            normal.normalize();
            faces.add(faceKey);
    ...
    const segments = [];
    const creaseAngles = [];
    let coplanarRemoved = 0;
    let boundaries = 0;
    let surfaceBoundaries = 0;
    let nonManifold = 0;
    ...
    for (const edge of adjacency.values()) {
        ...
        // Real open boundaries and visible material transitions survive the angle slider.
        const boundary = edge.count === 1;
        if (!boundary && !edge.surfaceBoundary && angle < planeEpsilon) { coplanarRemoved++; continue; }
        if (boundary) boundaries++;
        if (edge.surfaceBoundary) surfaceBoundaries++;
    ...
    diagnostics = { triangles, duplicates, degenerate, coplanarRemoved, boundaries, surfaceBoundaries, nonManifold, tolerance };
    // Maps/source arrays are local and collectable. Slider changes only filter compact cached candidates.
}
...
self.onmessage = ({ data }) => {
    try {
        ...
        self.postMessage({ positions, diagnostics }, [positions.buffer]);
    } catch (error) {
